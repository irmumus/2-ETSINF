...
public class EditorPredictivo extends ABB<String> {
    ...
    public ListaConPI<String> recuperarSucesores(String prefijo, int n) {       
        ...
        //Si prefijo está en el ABB lo inserto
        if(this.recuperar(prefijo) != null) {
            res.insertar(prefijo);
        }
        //Señalamos al siguiente
        String siguiente = prefijo;
        while(res.talla()<n){ //Máximo los n siguientes sucesores con el prefijo
            siguiente = sucesor(siguiente);
            //Si la cadena no acaba o si el siguiente empieza con prefijo
            if(siguiente != null && siguiente.startsWith(prefijo)){
                res.insertar(siguiente);
            } else {return res;}
        }
	return res;	
    }
