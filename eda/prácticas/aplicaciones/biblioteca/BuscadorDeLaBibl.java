...
public class BuscadorDeLaBibl {
    ...
    public ListaConPI<Termino> terminosUnicos() {
        ListaConPI<Termino> listaTerminos= index.claves();
        ListaConPI<Termino> res = new LEGListaConPI<Termino>();
        listaTerminos.inicio();
        while(!listaTerminos.esFin()) {
            Termino ter = listaTerminos.recuperar();
            ListaConPI<Posting> listaPosting = index.recuperar(ter);
            if(listaPosting.talla() == 1) {
                res.insertar(ter);
            }
            listaTerminos.siguiente();
        }
