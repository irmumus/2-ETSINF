...
public abstract class Grafo{
    ...
    public Arista[] kruskal() {       
        ...
        ForestUFSet fore = new ForestUFSet(numVertices());
        
        //Insertamos las aristas en la cola de prioridad
        for(int i = 0; i <numVertices(); i++){
            ListaConPI<Adyacente> lista = adyacentesDe(i);
            lista.inicio();
            while(!lista.esFin()) {
                Adyacente adya = lista.recuperar();
                Arista aInsertar = new Arista(i, adya.getDestino(), adya.getPeso());
                aristasFactibles.insertar(aInsertar);
                lista.siguiente();
            }
        }
        ...
        while(cardinal<numVertices()-1 && !aristasFactibles.esVacia()){
            ...
            int origen = fore.find(arista.getOrigen());
            int destino = fore.find(arista.getDestino());
            if(origen!=destino) {
                Eprima[cardinal++]=arista;
                fore.union(origen,destino);
            }
        }
        if(cardinal == numVertices()-1) { return Eprima;}
        else{return null;}
    }
}
