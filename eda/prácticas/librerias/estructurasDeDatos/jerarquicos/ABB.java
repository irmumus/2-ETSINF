...
public class ABB<E extends Comparable<E>> {
    ...
    protected NodoABB<E> construirEquilibrado(E[] v, int ini, int fin) {
        NodoABB<E> izq = null;
        NodoABB<E> der = null;
        int valorMediana =(fin+ini)/2;
        if(ini<fin) {
            izq = construirEquilibrado(v, ini, valorMediana-1);
            der = construirEquilibrado(v, valorMediana+1, fin);
        }
        NodoABB<E> res = new NodoABB<E>(v[valorMediana], izq, der);
        return res;
    }
    ...
    public void reconstruirEquilibrado() {        
        E[] newArrayEq = this.toArrayInOrden();
        raiz = construirEquilibrado(newArrayEq,0, newArrayEq.length-1);
    }
